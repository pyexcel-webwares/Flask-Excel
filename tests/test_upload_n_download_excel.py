...
class TestExcelResponse:
    ...
    def test_download(self):
        for upload_file_type in FILE_TYPE_MIME_TABLE.keys():
            ...
            for download_file_type in FILE_TYPE_MIME_TABLE.keys():
                ...
                io = pe.save_as(
                    dest_file_type=upload_file_type, array=self.data
                )
                if not PY2:
                ...
                sheet = pe.get_sheet(
                    file_type=download_file_type, file_content=response.data
                )
                sheet.format(int)
    ...
    def test_override_file_name(self):
        ...
        self._download_and_verify_file_name(file_name, url_encoded_file_name)
    ...
    def _download_and_verify_file_name(self, file_name, expected_file_name):
        for file_type in FILE_TYPE_MIME_TABLE.keys():
            response = self.app.post(
                "/file_name/%s/%s" % (file_type, file_name)
            )
